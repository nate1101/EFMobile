...
class Sponsor extends Component {
  ...
  fetchSponsor = async token => {
    const { navigation, userId } = this.props;
    const sponsorId = navigation.getParam("sponsorId", "0");

    try {
      const api = `${URLS.sponsor}/${sponsorId}`;
    ...
    return (
      <Container style={styles.container}>
        <View>
          <ListItem thumbnail>
            <Left>
              <Thumbnail round large source={{ uri: data.profilePic }} />
            </Left>
            <Body>
              <View>
                <Text style={styles.exhibitorName}>{data.sponsorName}</Text>
              </View>
            </Body>
          </ListItem>
          ...
          <ListItem>
            <View>
              ...
              {data.address2 ? <Text>{data.address2}</Text> : <View />}
              <Text>
          ...
          <ListItem>
            <View>
              ...
              <Button
                ...
                title="SponsorExhibitor Contact"
                style={{ color: "blue" }}
              >
                <Icon name="ios-mail" />
                <Text>{data.contactEmail}</Text>
          ...
          <ListItem>
            <View>
              <Button
                transparent
                onPress={() => Linking.openURL(`tel:${data.contactPhone}`)}
              >
                <Icon name="ios-phone-portrait" />
                <Text>{this.formatPhoneNumber(data.contactPhone)}</Text>
              </Button>
            </View>
          </ListItem>
        </View>
